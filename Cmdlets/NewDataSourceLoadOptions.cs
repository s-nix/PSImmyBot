...
public class NewDataSourceLoadOptions : Cmdlet {
    [Parameter(Mandatory = true)] public required bool AllowAsyncOverSync { get; set; }
    ...
    [Parameter(Mandatory = true)] public required bool IsCountQuery { get; set; }
    [Parameter(Mandatory = true)] public required bool IsSummaryQuery { get; set; }
    ...
    [Parameter(Mandatory = true)] public required bool RequireGroupCount { get; set; }
    [Parameter(Mandatory = true)] public required bool RequireTotalCount { get; set; }
    [Parameter(Mandatory = false)] public ICollection<string>? Select { get; set; }
    [Parameter(Mandatory = true)] public required int Skip { get; set; }
    ...
    [Parameter(Mandatory = true)] public required int Take { get; set; }
    [Parameter(Mandatory = false)] public ICollection<SummaryInfo>? TotalSummary { get; set; }

    protected override void ProcessRecord() {
        DataSourceLoadOptions outputObject = new(AllowAsyncOverSync, DefaultSort, ExpandLinqSumType, Filter, Group, GroupSummary, IsCountQuery, IsSummaryQuery, PaginateViaPrimaryKey, PreSelect, PrimaryKey, RemoteGrouping, RemoteSelect, RequireGroupCount, RequireTotalCount, Select, Skip, Sort, SortByPrimaryKey, StringToLower, Take, TotalSummary);
