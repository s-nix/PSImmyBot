...
[Cmdlet(VerbsCommon.New, "MethodInfo")]
public class NewMethodInfo : Cmdlet {
    ...
    [Parameter(Mandatory = true)] public required ParameterInfo ReturnParameter { get; set; }
    [Parameter(Mandatory = true)] public required Type ReturnType { get; set; }
    [Parameter(Mandatory = true)] public required ICustomAttributeProvider ReturnTypeCustomAttributes { get; set; }

    protected override void ProcessRecord() {
        MethodInfo outputObject = new(Attributes, CallingConvention, ContainsGenericParameters, CustomAttributes, DeclaringType, IsAbstract, IsAssembly, IsCollectible, IsConstructedGenericMethod, IsConstructor, IsFamily, IsFamilyAndAssembly, IsFamilyOrAssembly, IsFinal, IsGenericMethod, IsGenericMethodDefinition, IsHideBySig, IsPrivate, IsPublic, IsSecurityCritical, IsSecuritySafeCritical, IsSecurityTransparent, IsSpecialName, IsStatic, IsVirtual, MemberType, MetadataToken, MethodHandle, MethodImplementationFlags, Module, Name, ReflectedType, ReturnParameter, ReturnType, ReturnTypeCustomAttributes);
