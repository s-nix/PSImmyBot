---
title: PreconsentCustomerTenantsRequest
---

# PreconsentCustomerTenantsRequest

| Detail | Value |
| --- | --- |
| Namespace | `PSImmyBot.Models` |
| Kind | Class |
| Assembly | `PSImmyBot` |
| Base Type | `object` |
| Implements | `IEquatable<PreconsentCustomerTenantsRequest>` |
| Source | `Models/PreconsentCustomerTenantsRequest.cs` |

## Overview
`PreconsentCustomerTenantsRequest` is a Class from `PSImmyBot.Models` and participates in ImmyBot API payloads generated by the module.

## Structure

| Property | Type | Nullable | Notes |
| --- | --- | --- | --- |
| CustomerPrincipalIds | `ICollection<string>` | Reference type (nullable) | — |
| PartnerPrincipalId | `string` | Reference type (nullable) | — |

## Serialization

- Namespace: `PSImmyBot.Models`
- Source file: `Models/PreconsentCustomerTenantsRequest.cs`
- Serializer: `System.Text.Json`

## Notes

This page was generated automatically from the compiled PSImmyBot assembly via `HelperScripts/GenerateDocs.ps1`. Regenerate after model changes to keep details current.
