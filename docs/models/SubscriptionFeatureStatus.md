---
title: SubscriptionFeatureStatus
---

# SubscriptionFeatureStatus

| Detail | Value |
| --- | --- |
| Namespace | `PSImmyBot.Models` |
| Kind | Enum |
| Assembly | `PSImmyBot` |
| Base Type | `Enum` |
| Implements | `IComparable`, `IConvertible`, `IFormattable`, `ISpanFormattable` |
| Source | `Models/SubscriptionFeatureStatus.cs` |

## Overview
`SubscriptionFeatureStatus` is a Enum from `PSImmyBot.Models` and participates in ImmyBot API payloads generated by the module.

### Underlying type

- `int`

## Members

| Name | Value |
| --- | --- |
| UnKnown | 0 |
| Active | 1 |
| Archived | 2 |
| Draft | 3 |

## Serialization

- Namespace: `PSImmyBot.Models`
- Source file: `Models/SubscriptionFeatureStatus.cs`
- Serializer: `System.Text.Json`

## Notes

This page was generated automatically from the compiled PSImmyBot assembly via `HelperScripts/GenerateDocs.ps1`. Regenerate after model changes to keep details current.
