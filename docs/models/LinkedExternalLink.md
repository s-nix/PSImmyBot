---
title: LinkedExternalLink
---

# LinkedExternalLink

| Detail | Value |
| --- | --- |
| Namespace | `PSImmyBot.Models` |
| Kind | Class |
| Assembly | `PSImmyBot` |
| Base Type | `object` |
| Implements | `IEquatable<LinkedExternalLink>` |
| Source | `Models/LinkedExternalLink.cs` |

## Overview
`LinkedExternalLink` is a Class from `PSImmyBot.Models` and participates in ImmyBot API payloads generated by the module.

## Structure

| Property | Type | Nullable | Notes |
| --- | --- | --- | --- |
| IsExternalClientLinkingEnabled | `bool` | No (value type) | — |
| IsExternalProviderInitializedFromThisProvider | `bool` | No (value type) | — |
| ProviderLinkId | `int` | No (value type) | — |
| ProviderLinkName | `string` | Reference type (nullable) | — |
| ProviderTypeId | `Guid` | No (value type) | — |

## Serialization

- Namespace: `PSImmyBot.Models`
- Source file: `Models/LinkedExternalLink.cs`
- Serializer: `System.Text.Json`

## Notes

This page was generated automatically from the compiled PSImmyBot assembly via `HelperScripts/GenerateDocs.ps1`. Regenerate after model changes to keep details current.
