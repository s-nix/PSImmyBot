---
title: NotificationAcknowledgement
---

# NotificationAcknowledgement

| Detail | Value |
| --- | --- |
| Namespace | `PSImmyBot.Models` |
| Kind | Enum |
| Assembly | `PSImmyBot` |
| Base Type | `Enum` |
| Implements | `IComparable`, `IConvertible`, `IFormattable`, `ISpanFormattable` |
| Source | `Models/NotificationAcknowledgement.cs` |

## Overview
`NotificationAcknowledgement` is a Enum from `PSImmyBot.Models` and participates in ImmyBot API payloads generated by the module.

### Underlying type

- `int`

## Members

| Name | Value |
| --- | --- |
| Unacknowledged | 0 |
| Acknowledged | 1 |
| Dismissed | 2 |

## Serialization

- Namespace: `PSImmyBot.Models`
- Source file: `Models/NotificationAcknowledgement.cs`
- Serializer: `System.Text.Json`

## Notes

This page was generated automatically from the compiled PSImmyBot assembly via `HelperScripts/GenerateDocs.ps1`. Regenerate after model changes to keep details current.
