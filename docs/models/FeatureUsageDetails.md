---
title: FeatureUsageDetails
---

# FeatureUsageDetails

| Detail | Value |
| --- | --- |
| Namespace | `PSImmyBot.Models` |
| Kind | Class |
| Assembly | `PSImmyBot` |
| Base Type | `object` |
| Implements | `IEquatable<FeatureUsageDetails>` |
| Source | `Models/FeatureUsageDetails.cs` |

## Overview
`FeatureUsageDetails` is a Class from `PSImmyBot.Models` and participates in ImmyBot API payloads generated by the module.

## Structure

| Property | Type | Nullable | Notes |
| --- | --- | --- | --- |
| FeatureTrackStartDateUtc | `DateTimeOffset?` | Yes (nullable value type) | — |
| ItemCount | `int?` | Yes (nullable value type) | — |
| MaxCount | `int?` | Yes (nullable value type) | — |
| MaxCountPerItem | `int?` | Yes (nullable value type) | — |
| ResetInterval | `FeatureResetInterval` | No (value type) | — |

## Serialization

- Namespace: `PSImmyBot.Models`
- Source file: `Models/FeatureUsageDetails.cs`
- Serializer: `System.Text.Json`

## Notes

This page was generated automatically from the compiled PSImmyBot assembly via `HelperScripts/GenerateDocs.ps1`. Regenerate after model changes to keep details current.
