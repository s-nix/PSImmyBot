---
title: ExcludeFromMaintenanceRequest
---

# ExcludeFromMaintenanceRequest

| Detail | Value |
| --- | --- |
| Namespace | `PSImmyBot.Models` |
| Kind | Class |
| Assembly | `PSImmyBot` |
| Base Type | `object` |
| Implements | IEquatable<[ExcludeFromMaintenanceRequest](ExcludeFromMaintenanceRequest.md)> |
| Source | `Models/ExcludeFromMaintenanceRequest.cs` |

## Overview
`ExcludeFromMaintenanceRequest` is a Class from `PSImmyBot.Models` and participates in ImmyBot API payloads generated by the module.

## Structure

| Property | Type | Nullable | Notes |
| --- | --- | --- | --- |
| Exclude | `bool` | No (value type) | — |

## Serialization

- Namespace: `PSImmyBot.Models`
- Source file: `Models/ExcludeFromMaintenanceRequest.cs`
- Serializer: `System.Text.Json`

## Notes

This page was generated automatically from the compiled PSImmyBot assembly via `HelperScripts/GenerateDocs.ps1`. Regenerate after model changes to keep details current.
