---
title: RunImmybotPersonRequestBody
---

# RunImmybotPersonRequestBody

| Detail | Value |
| --- | --- |
| Namespace | `PSImmyBot.Models` |
| Kind | Class |
| Assembly | `PSImmyBot` |
| Base Type | `object` |
| Implements | IEquatable<[RunImmybotPersonRequestBody](RunImmybotPersonRequestBody.md)> |
| Source | `Models/RunImmybotPersonRequestBody.cs` |

## Overview
`RunImmybotPersonRequestBody` is a Class from `PSImmyBot.Models` and participates in ImmyBot API payloads generated by the module.

## Structure

| Property | Type | Nullable | Notes |
| --- | --- | --- | --- |
| MaintenanceTaskParameterValueOverrides | IDictionary<string, IDictionary<string, [DeploymentParameterValue](DeploymentParameterValue.md)>> | Reference type (nullable) | — |
| PersonId | `int` | No (value type) | — |

## Serialization

- Namespace: `PSImmyBot.Models`
- Source file: `Models/RunImmybotPersonRequestBody.cs`
- Serializer: `System.Text.Json`

## Notes

This page was generated automatically from the compiled PSImmyBot assembly via `HelperScripts/GenerateDocs.ps1`. Regenerate after model changes to keep details current.
