...
function ConvertFrom-SwaggerPatch {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true)]
        [string]$SwaggerFilePath
    ...
    $endpointPaths = $swaggerJson.paths.PSObject.Members.Name | Where-Object { $_ -like "/api*" }
    foreach ($endpointPath in $endpointPaths) {
        if (!$swaggerJson.paths.$endpointPath.patch) {
            continue
        }

        $payloadPrimitiveType = $swaggerJson.paths.$endpointPath.patch.requestBody.content.'application/json'.schema.type
        $payloadObjectType = $swaggerJson.paths.$endpointPath.patch.requestBody.content.'application/json'.schema.'$ref' | Split-Path -Leaf -ea 0
        $payloadObjectListType = $swaggerJson.paths.$endpointPath.patch.requestBody.content.'application/json'.schema.items.'$ref' | Split-Path -Leaf -ea 0
        $hasPayload = $payloadPrimitiveType -or $payloadObjectType -or $payloadObjectListType
        $payloadType = if ($payloadPrimitiveType -and -not $payloadObjectType -and -not $payloadObjectListType) {
            switch ($payloadPrimitiveType) {
                "string" { "string" }
        ...
        $returnPrimitiveType = $swaggerJson.paths.$endpointPath.patch.responses.'200'.content.'application/json'.schema.type
        $returnObjectType = $swaggerJson.paths.$endpointPath.patch.responses.'200'.content.'application/json'.schema.'$ref' | Split-Path -Leaf -ea 0
        $returnObjectListType = $swaggerJson.paths.$endpointPath.patch.responses.'200'.content.'application/json'.schema.items.'$ref' | Split-Path -Leaf -ea 0
        $hasReturn = $returnPrimitiveType -or $returnObjectType -or $returnObjectListType
        ...
        $parameters = $swaggerJson.paths.$endpointPath.patch.parameters
        $cmdletPayload =
        @"
using System.Management.Automation;
using PSImmyBot.Models;
...
[Cmdlet("Update", "$targetMethodName")]
public class Update$targetMethodName : Cmdlet {
$(foreach ($param in $parameters) {
    $paramName = $param.name
    $paramType = $param.schema.'$ref' | Split-Path -Leaf -ea 0
    $paramRequired = $param.required -eq $true
...
$(if ($hasPayload -and $hasReturn) {
    "        $targetReturnType response = ImmyBotApiService.Patch<$payloadType, $targetReturnType>(endpoint.TrimEnd('?').TrimEnd('&'), PayloadBody).GetAwaiter().GetResult();
        WriteObject(response);"
} elseif ($hasPayload -and -not $hasReturn) {
    "        ImmyBotApiService.Patch(endpoint.TrimEnd('?').TrimEnd('&'), PayloadBody).GetAwaiter().GetResult();"
} elseif (-not $hasPayload -and $hasReturn) {
    "        $targetReturnType response = ImmyBotApiService.Patch<$targetReturnType>(endpoint.TrimEnd('?').TrimEnd('&')).GetAwaiter().GetResult();
        WriteObject(response);"
} else {
...
"@
        $cmdletFilePath = "Cmdlets/Update$targetMethodName.cs"
        $cmdletPayload | Out-File -FilePath $cmdletFilePath -Encoding UTF8
    }
}

function ConvertFrom-SwaggerPut {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true)]
        [string]$SwaggerFilePath
    ...
    $endpointPaths = $swaggerJson.paths.PSObject.Members.Name | Where-Object { $_ -like "/api*" }
    foreach ($endpointPath in $endpointPaths) {
        if (!$swaggerJson.paths.$endpointPath.put) {
            continue
        }

        $payloadPrimitiveType = $swaggerJson.paths.$endpointPath.put.requestBody.content.'application/json'.schema.type
        $payloadObjectType = $swaggerJson.paths.$endpointPath.put.requestBody.content.'application/json'.schema.'$ref' | Split-Path -Leaf -ea 0
        $payloadObjectListType = $swaggerJson.paths.$endpointPath.put.requestBody.content.'application/json'.schema.items.'$ref' | Split-Path -Leaf -ea 0
        $hasPayload = $payloadPrimitiveType -or $payloadObjectType -or $payloadObjectListType
        $payloadType = if ($payloadPrimitiveType -and -not $payloadObjectType -and -not $payloadObjectListType) {
            switch ($payloadPrimitiveType) {
                "string" { "string" }
        ...
        $returnPrimitiveType = $swaggerJson.paths.$endpointPath.put.responses.'200'.content.'application/json'.schema.type
        $returnObjectType = $swaggerJson.paths.$endpointPath.put.responses.'200'.content.'application/json'.schema.'$ref' | Split-Path -Leaf -ea 0
        $returnObjectListType = $swaggerJson.paths.$endpointPath.put.responses.'200'.content.'application/json'.schema.items.'$ref' | Split-Path -Leaf -ea 0
        $hasReturn = $returnPrimitiveType -or $returnObjectType -or $returnObjectListType
        ...
        $parameters = $swaggerJson.paths.$endpointPath.put.parameters
        $cmdletPayload =
        @"
using System.Management.Automation;
using PSImmyBot.Models;
...
[Cmdlet("Push", "$targetMethodName")]
public class Push$targetMethodName : Cmdlet {
$(foreach ($param in $parameters) {
    $paramName = $param.name
    $paramType = $param.schema.'$ref' | Split-Path -Leaf -ea 0
    $paramRequired = $param.required -eq $true
...
$(if ($hasPayload -and $hasReturn) {
    "        $targetReturnType response = ImmyBotApiService.Put<$payloadType, $targetReturnType>(endpoint.TrimEnd('?').TrimEnd('&'), PayloadBody).GetAwaiter().GetResult();
        WriteObject(response);"
} elseif ($hasPayload -and -not $hasReturn) {
    "        ImmyBotApiService.Put(endpoint.TrimEnd('?').TrimEnd('&'), PayloadBody).GetAwaiter().GetResult();"
} elseif (-not $hasPayload -and $hasReturn) {
    "        $targetReturnType response = ImmyBotApiService.Put<$targetReturnType>(endpoint.TrimEnd('?').TrimEnd('&')).GetAwaiter().GetResult();
        WriteObject(response);"
} else {
    "        ImmyBotApiService.Put(endpoint.TrimEnd('?').TrimEnd('&')).GetAwaiter().GetResult();"
})
    }

}
"@
        $cmdletFilePath = "Cmdlets/Push$targetMethodName.cs"
        $cmdletPayload | Out-File -FilePath $cmdletFilePath -Encoding UTF8
    }
}

function ConvertFrom-SwaggerDelete {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true)]
        [string]$SwaggerFilePath
    ...
    $endpointPaths = $swaggerJson.paths.PSObject.Members.Name | Where-Object { $_ -like "/api*" }
    foreach ($endpointPath in $endpointPaths) {
        if (!$swaggerJson.paths.$endpointPath.delete) {
            continue
        }

        $payloadPrimitiveType = $swaggerJson.paths.$endpointPath.delete.requestBody.content.'application/json'.schema.type
        $payloadObjectType = $swaggerJson.paths.$endpointPath.delete.requestBody.content.'application/json'.schema.'$ref' | Split-Path -Leaf -ea 0
        $payloadObjectListType = $swaggerJson.paths.$endpointPath.delete.requestBody.content.'application/json'.schema.items.'$ref' | Split-Path -Leaf -ea 0
        $hasPayload = $payloadPrimitiveType -or $payloadObjectType -or $payloadObjectListType
        $payloadType = if ($payloadPrimitiveType -and -not $payloadObjectType -and -not $payloadObjectListType) {
            switch ($payloadPrimitiveType) {
                "string" { "string" }
        ...
        $returnPrimitiveType = $swaggerJson.paths.$endpointPath.delete.responses.'200'.content.'application/json'.schema.type
        $returnObjectType = $swaggerJson.paths.$endpointPath.delete.responses.'200'.content.'application/json'.schema.'$ref' | Split-Path -Leaf -ea 0
        $returnObjectListType = $swaggerJson.paths.$endpointPath.delete.responses.'200'.content.'application/json'.schema.items.'$ref' | Split-Path -Leaf -ea 0
        $hasReturn = $returnPrimitiveType -or $returnObjectType -or $returnObjectListType
        ...
        $parameters = $swaggerJson.paths.$endpointPath.delete.parameters
        $cmdletPayload =
        @"
using System.Management.Automation;
using PSImmyBot.Models;
...
[Cmdlet("Remove", "$targetMethodName")]
public class Remove$targetMethodName : Cmdlet {
$(foreach ($param in $parameters) {
    $paramName = $param.name
    $paramType = $param.schema.'$ref' | Split-Path -Leaf -ea 0
    $paramRequired = $param.required -eq $true
...
$(if ($hasPayload -and $hasReturn) {
    "        $targetReturnType response = ImmyBotApiService.Delete<$payloadType, $targetReturnType>(endpoint.TrimEnd('?').TrimEnd('&'), PayloadBody).GetAwaiter().GetResult();
        WriteObject(response);"
} elseif ($hasPayload -and -not $hasReturn) {
    "        ImmyBotApiService.Delete(endpoint.TrimEnd('?').TrimEnd('&'), PayloadBody).GetAwaiter().GetResult();"
} elseif (-not $hasPayload -and $hasReturn) {
    "        $targetReturnType response = ImmyBotApiService.Delete<$targetReturnType>(endpoint.TrimEnd('?').TrimEnd('&')).GetAwaiter().GetResult();
        WriteObject(response);"
} else {
...
"@
        $cmdletFilePath = "Cmdlets/Remove$targetMethodName.cs"
        $cmdletPayload | Out-File -FilePath $cmdletFilePath -Encoding UTF8
    }
}
